...
⚡ lqc-tokens advisory
  ...
  Mitigation: /lqc-tokens:docker-advisor — offload to Postgres, query instead of paste
```
...
```bash
cp -r lqc-tokens .claude/plugins/
# restart Claude Code
```
...
|-------|--------|---------|
| `setup` | `/lqc-tokens:setup` | Inject CLAUDE.md entry, capture baseline, verify Docker |
...
| `docker-advisor` | `/lqc-tokens:docker-advisor` | DB strategy + shared base container + namespace |
...
| `eval` | `/lqc-tokens:eval` | Token savings report vs. pre-install baseline |
...
`~/.claude/lqc-tokens.local.md` (auto-managed, gitignored):
